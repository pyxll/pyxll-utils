...
# win32com is required for the code that cleans up the cache in response to Excel events.
...
def _setup_event_handler(cache):
    ...
    if cache not in _event_handlers:
        ...
        app_handler = win32com.client.DispatchWithEvents(xl, ObjectCacheApplicationEventHandler)
